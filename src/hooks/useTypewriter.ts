...
export const useTypewriter = (targetText: string, isThinking: boolean) => {
  ...
  // INCREASED to 50ms (20fps) to allow more time for main thread to handle layout/markdown parsing.
  // Previous 25ms (40fps) caused layout thrashing on heavy Markdown pages.
  const TICK_RATE = 50;
  ...
  const loop = useCallback(() => {
      ...
      // Adjusted for 50ms tick rate (approx 2x chars compared to 25ms).
      // We also CAP the max charsToAdd to prevent massive layout shifts in one frame.
      const remainingChars = targetLen - currentLength.current;
      let charsToAdd = 2; // Higher base speed

      // Acceleration logic:
      if (remainingChars > 1500) charsToAdd = 200;    // Massive catch-up (Capped to prevent freeze)
      else if (remainingChars > 800) charsToAdd = 100; // Very Fast
      else if (remainingChars > 400) charsToAdd = 60; // Fast
      else if (remainingChars > 150) charsToAdd = 40; // Moderate Fast
      else if (remainingChars > 50) charsToAdd = 20;   // Cruising speed
      else if (remainingChars > 20) charsToAdd = 10;   // Decent pace
      else charsToAdd = 4; // Base speed
